export const s = {
	deleteAllButton: /*tw*/ 'flex items-center text-branded text-[14px] font-bold mb-5',
	crossIcon: /*tw*/ 'w-[14px] mr-2',

	list: /*tw*/ 'flex space-x-5 pb-5',
	listItem: /*tw*/ 'relative min-w-[335px] h-[217px] border border-dimmerText py-2',
	imgWrapper: /*tw*/ 'h-[150px] w-full flex flex-col justify-center items-center',
	image: /*tw*/ 'w-auto h-full',
	removeItemButton: /*tw*/ 'absolute top-2.5 right-2.5 text-dimText',
	itemName: /*tw*/ 'block text-center text-[17px] font-medium',
}
